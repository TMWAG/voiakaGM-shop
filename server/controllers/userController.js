  ...
  static async registration(req, res, next) {
    try {
      ...
      if (
        ...
        return next(
          ApiError.badRequest("Введённые данные не удовлетворяют требованиям")
        );
      }
  ...
  static async getOneById(req, res, next) {
    try {
      ...
      const user = await User.findOne({
        where: { id },
        attributes: ["id", "name", "surname", "vkLink", "tgLink", "createdAt"],
      });
  ...
  static async addTGLinkById(req, res, next) {
    try {
      ...
      if (!inputValidation.checkTgLink(tgLink)) {
        return next(
          ApiError.badRequest("Указано неверное имя пользователя Telegram")
        );
      }
      if (await User.findOne({ where: { tgLink } })) {
        return next(
          ApiError.badRequest(
            "Данное имя пользователя telegram привязано к другому аккаунту"
          )
        );
      }
  ...
  static async addVKLinkById(req, res, next) {
    try {
      ...
      if (!inputValidation.checkVKLink(vkLink)) {
        return next(
          ApiError.badRequest("Указана недействительная ссылка на ВК")
        );
      }
      if (await User.findOne({ where: { vkLink } })) {
        return next(
          ApiError.badRequest("Этот аккаунт ВК уже привязан к другому аккаунту")
        );
      }
  ...
  static async changePhoneById(req, res, next) {
    try {
      ...
      if (await User.findOne({ where: { phone } })) {
        return next(
          ApiError.badRequest(
            "Данный номер телефона привязан к другому аккаунту"
          )
        );
      }
  ...
  static async changePasswordById(req, res, next) {
    try {
      ...
      if (!inputValidation.checkPassword(newPassword)) {
        return next(
          ApiError.badRequest("Введённый пароль не соответствует требованиям")
        );
      }
      const user = await User.findOne({ where: { id } });
      if (await bcrypt.compare(newPassword, user.password)) {
        return next(
          ApiError.badRequest("Новый пароль должен отличаться от старого")
        );
      }
