...
module.exports = class OrderController {
  ...
  static async getAllByUserId(req, res, next) {
    try {
      ...
      if (
        !(userId == req.user.id) ||
        !(req.user.roleId >= process.env.MODERATOR_ROLE_ID)
      ) {
        return next(
          ApiError.forbidden(
            "Просматривать заказы может только сам пользователь или администрация"
          )
        );
      }
  ...
  static async addTrackNoById(req, res, next) {
    try {
      ...
      const order = await Order.update({ trackNo }, { where: { id } });
      return res.json(order);
    } catch (e) {
      next(ApiError.badRequest(e.message));
    }
  ...
  static async addUserAddressById(req, res, next) {
    try {
      ...
      const order = await Order.update({ userAddressId }, { where: { id } });
      return res.json(order);
    } catch (e) {
      next(ApiError.badRequest(e.message));
    }
