...
module.exports = class ParameterController{
    ...
    static async getAll(req, res, next){
        try {
            const parameters = await Parameter.findAll({attributes: ["id", "parameterName"]});
    ...
    static async getOneById(req, res, next){
        try {
            const {id} = req.params;
            const parameter = await Parameter.findOne({attributes: ["id", "parameterName"], where: {id}});
            return res.json(parameter)
        } catch (e) {
            next(ApiError.badRequest(e.message));
        }
    ...
    static async changeNameById(req, res, next){
        try {
            ...
            const parameter = await Parameter.update({parameterName}, {where:{id}});
            return res.json(parameter);
        } catch (e) {
            next(ApiError.badRequest(e.message));
        }
