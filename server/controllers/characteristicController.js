...
module.exports = class CharacteristicController{
    static async create(req, res, next){
        try {
            ...
            const characteristic = await Characteristic.create({parameterId, productId, value});
    ...
    static async getOneById(req, res, next){
        try {
            const {id} = req.params;
            const characteristic = await Characteristic.findOne({where: {id}, attributes: ["id", "value", "productId", "parameterId"]});
            return res.json(characteristic);
        } catch (e) {
            next(ApiError.badRequest(e.message));
        }
    ...
    static async getAllByProductId(req, res, next){
        try {
            ...
            const characteristics = await Characteristic.findAll({where: {productId}, attributes: ["id", "value", "parameterId"]});
    ...
    static async changeParameterById(req, res, next){
        try {
            ...
            const characteristic = await Characteristic.update({parameterId}, {where: {id}});
            return res.json(characteristic);
        } catch (e) {
            next(ApiError.badRequest(e.message));
        }
    ...
    static async changeValueById(req, res, next){
        try {
            ...
            const characteristic = await Characteristic.update({value}, {where: {id}});
            return res.json(characteristic);
        } catch (e) {
            next(ApiError.badRequest(e.message));
        }
    ...
    static async deleteAllByProductId(req, res, next){ 
        try {
            ...
            const characteristics = await Characteristic.destroy({where: {productId}});
            return res.json(characteristics);
        } catch (e) {
            next(ApiError.badRequest(e.message));
        }
